  ...
  return (
    <article className={`relative flex flex-col ${className}`}>
      ...
      <div className="flex items-center gap-3 order-[-1] mb-[0.5rem]">
        {category && (
          <span className="inline-block px-3 py-1 rounded-full bg-primary/10 text-primary text-[0.6875rem] font-medium uppercase tracking-wide">
            {category}
          </span>
        )}
        <time className="uppercase text-muted-foreground text-[0.75rem]">
          {publishDate}
        </time>
      </div>

      <h3 className="mt-[0.1875rem] text-[1.375rem] md:text-[1.6875rem] leading-[1.4] tracking-[-0.02em]">
        <Link
          to={`/article/${slug}`}
      ...
      {description && (
        <p className="mt-3 text-[0.9375rem] leading-relaxed text-muted-foreground italic">
          {description}
        </p>
      )}
    </article>
